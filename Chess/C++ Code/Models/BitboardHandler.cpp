...
#include "../Headers/BitboardHandler.hpp"

BitboardHandler::BitboardHandler(Board *board, Color color, CentralBitboard *centralBitboard) {
//        this->board = board;
//        this->color = color;
//        this->centralBitboard = centralBitboard;
}

BitboardHandler::void clearAllMovesBitboards() {
...
BitboardHandler::void updateArray(bool whiteArray[8][8], bool blackArray[8][8], U64 bitboard) {
//        for(LinkedList<Position>::Node *node = this->centralBitboard->computePositionsFromBitboard(bitboard)->head; node != nullptr; node = node->next) {
//            Index index = this->board->positionToIndex(*node->value);
//            
//            if (this->color == white) {
...
BitboardHandler::void updatePresenceAndMovesArrays() {
//        if (this->color == white) {
